...
export function StaffPageClient() {
  ...
  const [editForm, setEditForm] = useState<{ full_name: string; role: Role }>({
    full_name: '',
    role: 'staff',
  })
  ...
  const openEditModal = (staff: StaffMember) => {
    setEditTarget(staff)
    setEditForm({ full_name: staff.full_name ?? '', role: staff.role })
  }
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    setSavingEdit(true)
    try {
      const res = await fetch(`/api/staff/${editTarget.staff_id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link
            href="/tips-admin"
            className="border border-aro-hairline hover:bg-aro-sand/60 text-aro-ink px-4 py-2 rounded-lg flex items-center space-x-2 transition-colors"
          >
      ...
      {/* Edit Modal (PLAN-34: name + role, reuses the existing PATCH route
          which already enforces the manager-can-only-touch-staff rule) */}
      {editTarget && (
        <div className="fixed inset-0 bg-aro-ink/60 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-sm w-full p-6">
            <h2 className="text-xl font-bold text-aro-ink mb-1">Edit Staff Member</h2>
            <p className="text-sm text-aro-muted mb-4">{editTarget.email}</p>
