  ...
  return (
    <div>
      <div className="mb-6 lg:mb-8">
        <h1 className="font-display text-2xl lg:text-4xl font-bold text-aro-ink">HQ Overview</h1>
        <p className="text-aro-muted mt-1 lg:mt-2 text-sm lg:text-lg">Every client, at a glance</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-6">
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-4 lg:p-6 shadow-lg border border-aro-hairline">
          <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-xs lg:text-sm text-aro-muted">Clients</p>
          </div>
          <p className="text-2xl lg:text-3xl font-bold text-aro-ink">{venues.length}</p>
        </div>

        <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-4 lg:p-6 shadow-lg border border-aro-hairline">
          <div className="flex items-center gap-3 mb-2">
            <Users className="w-5 h-5 lg:w-6 lg:h-6 text-aro-muted" />
            <p className="text-xs lg:text-sm text-aro-muted">Members</p>
          </div>
          <p className="text-2xl lg:text-3xl font-bold text-aro-ink">{totalMembers}</p>
        </div>

        <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-4 lg:p-6 shadow-lg border border-aro-hairline">
          <div className="flex items-center gap-3 mb-2">
            <Activity className="w-5 h-5 lg:w-6 lg:h-6 text-aro-muted" />
            <p className="text-xs lg:text-sm text-aro-muted">Visits this week</p>
          </div>
          <p className="text-2xl lg:text-3xl font-bold text-aro-ink">{visitsThisWeekDisplay}</p>
          {visitsDeltaPct !== null && (
        ...
        <div className="bg-aro-plum/10 backdrop-blur-xl rounded-2xl p-4 lg:p-6 shadow-lg border border-aro-plum/30">
          <div className="flex items-center gap-3 mb-2">
            <Inbox className="w-5 h-5 lg:w-6 lg:h-6 text-aro-plum" />
            <p className="text-xs lg:text-sm text-aro-ink">New leads</p>
          </div>
          <p className="text-2xl lg:text-3xl font-bold text-aro-ink">{newLeadsCount}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <Link
          href="/clients"
          className="flex items-center justify-center gap-2 bg-aro-terra text-white font-semibold py-2.5 px-4 rounded-xl shadow-md hover:shadow-lg transition-all text-sm"
        >
        ...
        <Link
          href="/leads"
          className="flex items-center justify-center gap-2 bg-white/80 border border-aro-hairline text-aro-ink-soft font-semibold py-2.5 px-4 rounded-xl shadow-sm hover:shadow-md transition-all text-sm"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-4 lg:p-6 shadow-lg border border-aro-hairline">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-aro-ink">Clients at a glance</h2>
            <Link
              href="/clients"
              className="text-xs text-aro-muted hover:text-aro-ink-soft flex items-center gap-1"
            >
              All clients <ArrowRight className="w-3 h-3" />
            </Link>
          </div>
          {venues.length === 0 ? (
            <p className="text-sm text-aro-muted py-6 text-center">
            ...
            <div className="space-y-2">
              {venues.slice(0, 8).map(v => (
                <div
                  key={v.venue_id}
                  className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-aro-sand/40"
                >
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-aro-ink truncate">{v.business_name}</p>
                    <p className="text-xs text-aro-muted">{v.slug}</p>
                  </div>
                  <div className="flex items-center gap-4 text-xs text-aro-muted flex-shrink-0">
        ...
        {/* Recent activity */}
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-4 lg:p-6 shadow-lg border border-aro-hairline">
          <h2 className="text-lg font-bold text-aro-ink mb-4">Recent activity</h2>
          {events.length === 0 ? (
            <p className="text-sm text-aro-muted py-6 text-center">
            ...
            <div className="space-y-2">
              {events.map(e => (
                <div
                  key={e.event_id}
                  className="flex items-center justify-between text-sm border-b border-aro-hairline pb-2 last:border-0"
                >
                  <div className="min-w-0">
                    <p className="text-aro-ink">{eventLabel(e.type)}</p>
                    {e.venue_id && venueNameById.get(e.venue_id) && (
                      <p className="text-xs text-aro-muted truncate">
                        {venueNameById.get(e.venue_id)}
                      </p>
                    )}
                  </div>
                  <span className="text-xs text-aro-muted flex-shrink-0 ml-3">
