...
**Dependencies** — needs: `OPENAI_API_KEY` (owner, minutes — resolved D-4,
overriding the strategic doc's §4.2/§9.4 Anthropic recommendation). Unlocks:
N2 site-copy, N3 digest delivery, N5's referral copy generation, and — most
importantly — the owner's daily habit that makes the eventual M-1 vendor
decision feel obvious instead of risky.
...
| D-17 | Stripe account model: single platform account vs. Stripe Connect (per-venue accounts) | ✅ **Resolved (owner, 2026-07-20)**   | Single account now (matches `lib/payments/adapters/stripe.ts` exactly as built — one global `STRIPE_SECRET_KEY`, zero extra dev work); defer Connect until ≥2 paying venues make "cafés keep their own rates" a live promise instead of a documentation line. Connect is deferred future scope, not a small follow-on — it needs an adapter rewrite to pass `stripeAccount` per call, plus a client onboarding flow and Stripe platform approval, none of which R2 as currently specced covers. | R2 (unblocked; build against single-account) |
