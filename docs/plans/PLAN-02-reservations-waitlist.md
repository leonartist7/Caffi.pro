...
-- "hours" is the venue's weekly booking window, keyed mon..sun; each value is
-- ["HH:MM","HH:MM"] (venue-local open/close) or null for closed that day, e.g.
--   {"mon":["08:00","20:00"], ..., "sun":null}
-- Default is null (unconfigured): availability returns NO slots until the owner
-- sets hours in the Phase 5 settings UI — a venue must never be bookable on
-- invented hours (master plan §3 rule 5: visible "needs setup", never a silent
-- fake). The seed gives The Roastery real hours so demos work immediately.
...
- `venues.reservation_config`: verified 2026-07-16 — the live anon/authenticated grant on `venues` is a **column list** (it does not include new columns automatically), so `reservation_config` stays server-only by omission. Do not add it to any public grant; the availability API exposes derived slot times, never the raw config.
...
2. **`create_reservation(p_venue_id UUID, p_client_uuid UUID, p_guest_name TEXT, p_guest_phone TEXT, p_guest_email TEXT, p_party_size INT, p_starts_at TIMESTAMPTZ, p_notes TEXT, p_member_id UUID, p_source TEXT) RETURNS reservations`**: validates `p_party_size` against `reservation_config.min_party`/`max_party`, validates `p_starts_at` is in the future, within `max_advance_days`, and inside the venue's `reservation_config.hours` window for that venue-local weekday (null hours or a closed day ⇒ raise the same catchable exception class as no-availability — the DB is the enforcement point, the availability API is only a convenience view), calls `find_available_table`; if `NULL`, raises a distinct, catchable exception (e.g. `RAISE EXCEPTION 'NO_AVAILABILITY' USING ERRCODE = 'P0001'`) so the API layer can return a friendly 409 rather than a generic 500; if a table is found, inserts the reservation row with `duration_minutes` from `reservation_config.default_duration_minutes`, status `'confirmed'`, and that `table_id`; and inserts the `reservation.created` row into `public.events` **inside the function, in the same transaction** — verified 2026-07-16: this is exactly what the ordering pattern does (`20260714100000_storefront_order_creation.sql` ~line 198 inserts `order.placed` into events itself). The API route must therefore NOT also call `emitEvent` for creation, or every booking double-logs. Idempotent: `client_uuid` replay (23505 on the unique constraint) returns the existing row, never a second one — and must not emit a second event.
...
- `app/api/reservations/availability/route.ts`: **GET** `?venue_slug=&party_size=&date=` (public) — returns `{ configured: boolean, slots: string[] }` of open slot start-times for that venue-local calendar day, computed by looping the day's slot grid (`slot_minutes` from config) between the venue's `reservation_config.hours` window for that weekday and checking table availability for each slot (one read-only conflict query over the day's reservations is fine — do NOT call the locking `find_available_table` in a loop for reads; `FOR UPDATE` is for the write path only). Hours were added to `reservation_config` in Phase 1 (verified 2026-07-16: no hours-shaped field existed anywhere before this plan — `venues`, `brand_kit`, and `loyalty_config` were all checked). If `hours` is null (unconfigured) or the day is closed, return `{ configured: false, slots: [] }` / `{ configured: true, slots: [] }` respectively — never invent a default open/close time.
...
- Middleware (verified 2026-07-16 — the earlier claim of a "public-route allowlist" was wrong; here is how it actually works): `middleware.ts` skips ALL `/api/*` paths outright, and page auth-gating lives in the route-group layouts, not in middleware — so `/api/reservations` needs no middleware change, and `/reserve/[slug]` is public automatically because it sits outside the gated `(dashboard)`/`(owner)` route groups. The ONE change middleware does need: the custom-domain rewrite (`middleware.ts` ~lines 52–56) currently passes through only `/shop/`, `/join`, `/pass` and rewrites everything else to `/shop/<slug>/<path>` — which would turn a custom-domain café's `/reserve` into a 404. Add a branch before that rewrite: if `pathname` is `/reserve` or starts with `/reserve/`, rewrite to `/reserve/${venue.slug}` instead.
...
- Events: add `reservation.created`, `reservation.status_changed`, `waitlist.joined`, `waitlist.status_changed` to `AroEventType` + `EVENT_LABELS`. Emission split (mirrors ordering): `reservation.created` is inserted by the `create_reservation` SQL function (Phase 2) — routes must not re-emit it; the other three are emitted from their routes via `void emitEvent(...)`.
...
- Booking-hours editing: a "Booking hours" card on the HQ Reservations page (or its settings section) — seven rows (mon–sun), each with a closed toggle + open/close time inputs, writing `reservation_config.hours` through a gated PATCH (extend the venue-update path used by Settings → General, `app/api/clients/[id]` PATCH, rather than a new endpoint — check its allowed-fields list and add `reservation_config`). Until hours are set, the HQ page shows a warm setup prompt and the guest widget shows "This café hasn't opened online bookings yet" — wired to the `configured: false` availability response from Phase 3.
...
- Seed additions (idempotent, fixed UUIDs, extend `supabase/seed/aro_dev_seed.sql`): give The Roastery's existing seeded tables real `capacity` values (2–3 different sizes so availability logic has something to differentiate), set its `reservation_config.hours` (e.g. 08:00–20:00 daily, Sunday closed), one demo reservation, one demo waitlist entry.
...
3. Book a table to full capacity for a given slot (across enough covers to exhaust every table sized for that party), then attempt one more booking for the same slot/party size → `NO_AVAILABILITY` / 409, not a 500.
   3b. Hours gating: a venue with `hours: null` returns `configured: false` and zero slots (widget shows the setup state, not a broken form); a request for a closed day returns zero slots; a booking POST for a time outside hours is rejected server-side even if the client somehow submits it.
