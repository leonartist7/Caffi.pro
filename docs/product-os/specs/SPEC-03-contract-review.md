...
Replay precedence: after locking the checkout operation, an existing exact-fingerprint operation returns the committed order and immutable delivery_order_context BEFORE current menu/zone/currency/quote-expiry repricing. Reject a changed fingerprint and never overwrite the context. The existing payment reservation/recovery and terminal-state guards still run; this exception cannot create a new order or dispatch and does not bypass payment safety. The current-pricing comparisons above apply only to first creation.
...
Refresh is two-step: request a new quote (no binding changes), then explicitly approve its exact provider cost and expected job version in dispatch/refresh approval; lock and recheck version and never-sent predicate atomically. Mismatches return 409. Increment job version, preserve old quote rows and write actor/reason/old/new quote audit. Owners/managers see job costs and sanitized attempt/error details; assigned-driver projection contains only task ID, confirmed milestone, readiness and minimum destination/contact; guest projection contains status, supported ETA/tracking freshness and no addresses, contacts, costs, job/connection IDs or exception internals. Event inbox has an index on connection_id/external_ref WHERE processed_at IS NULL.
...
Review regression cases additionally cover changed venue currency/zone fee/menu after identical checkout, changed structured address with reused checkout UUID, expired queued quote refresh with stale expected version, stalled create beyond lease (only one create authorized), early webhook across restart/before reference attachment, and same-venue unassigned/revoked driver detail reads.
