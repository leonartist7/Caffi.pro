...
At growth code head `0e8eecc`, first-visit referral credit uses an additive leased outbox and tenant-coherent visit constraint, and review analytics require the guest tracking token. Independent Astra xhigh static review approved the exact code head. [CI 36002940033](https://github.com/leonartist7/Caffi.pro/actions/runs/36002940033) passes type, lint, 53 unit tests, docs, isolated build, nine disposable SQL suites and seven Chromium tests. Critical production dependency audit still fails. No vendor sandbox, paid AI or live verification is claimed.
