...
## 2026-09-24 SPEC-05 checkpoint

The POS boundary and local simulator are on the scoped codex/spec-05-pos-connection branch based on open draft PR #87, not main. Paid orders receive a durable POS intent, and connected-order acceptance requires acknowledgement. Local types, lint and unit tests pass; database/browser, vendor sandbox and live proof remain unrun. Square and Clover are candidate vendors only; first-client access is not established. See the [SPEC-05 handoff](handoffs/HANDOFF-2026-09-24-spec-05.md).
