...
The scoped growth branch adds replay-safe counter redemption, refunded-earn reversal, recoverable paid-offer work and grounded Creative Studio generation with disabled-by-default venue budgets. Local types, lint and unit tests pass; disposable SQL/concurrency, browser, paid Gateway and live proof remain pending. Existing guest identity still requires an explicit member binding; no contact inference or outbound campaign action was added. See the [SPEC-06 handoff](handoffs/HANDOFF-2026-09-24-spec-06.md).
