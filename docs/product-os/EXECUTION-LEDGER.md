...
| SPEC-02 | Connected sales journey | Fingerprinted checkout reservation, checkout recovery, scoped guest tracking, venue-bound local test payment, provider-event reconciliation ledger, active kitchen membership recheck, opening-hours enforcement and resettable fixture/SQL contract on `codex/phase-02-connected-ordering` | Types, strict lint, 27 Vitest tests, docs, and isolated build pass in GitHub Actions run 35697491599; local SQL/browser blocked by absent tooling | Not run | Not run | Review; Astra xhigh static review accepted. Critical dependency audit and local migration/RLS/browser evidence remain required; F-02 blocks hosted demo |
