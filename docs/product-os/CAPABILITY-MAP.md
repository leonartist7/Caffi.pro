...
The 2026-09-24 SPEC-06 draft [PR #89](https://github.com/leonartist7/Caffi.pro/pull/89) adds an opt-in Gateway boundary, venue facts and budget audit, replay-safe redemption, refunded-earn reversal, durable paid-offer and first-visit referral work, and guest-token-gated review analytics. The owner member view uses only explicit same-venue member IDs to read orders, reservations and delivery jobs. [CI 36002940033](https://github.com/leonartist7/Caffi.pro/actions/runs/36002940033) passes type/lint/53 unit/docs/build, nine disposable SQL suites and seven Chromium tests, including same-name tenant isolation and referral recovery. Independent Astra xhigh static review approved the exact `0e8eecc` code head. Genuine concurrent sessions, revoked-owner/mobile paths, paid Gateway sandbox and live results remain pending; baseline rows above are historical.
