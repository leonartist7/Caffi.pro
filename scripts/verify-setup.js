...
async function testDatabaseFunctions() {
  ...
  try {
    ...
    try {
      const { data: coupons } = await supabase.from('coupons').select('code, tenant_id').limit(1)
      ...
      if (coupons && coupons.length > 0) {
        ...
        if (error) {
          ...
          logTest('validate_coupon()', 'warn', `Coupon invalid: ${data[0].error_message}`)
        }
      } else {
...
async function testAuthConfiguration() {
  ...
  try {
    ...
    const { data, error } = await supabase.from('super_admins').select('*').limit(0)
...
async function runAllTests() {
  ...
  console.log(
    `ℹ️  Info:     ${results.tests.length - results.passed - results.failed - results.warnings}`
  )
  console.log('\n')
