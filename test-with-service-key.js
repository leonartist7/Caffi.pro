...
async function testWithServiceKey() {
  ...
  try {
    ...
    if (usersError) {
      ...
      users.forEach(u => console.log(`   - ${u.full_name}: ${u.loyalty_points} pts (${u.loyalty_tier})`))
    }
    ...
    if (couponsError) {
      ...
      coupons.forEach(c => console.log(`   - ${c.code}: ${c.discount_value}${c.discount_type === 'percentage' ? '%' : '€'} off`))
    }
